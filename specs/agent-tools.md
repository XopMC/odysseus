...
- `src.tool_approvals` seals an opaque, expiring exact first action plus server-only selected tools and continuation query to owner, session, origin run, tool content, workspace, capability snapshot, and—when relevant—document id/version/content digest. Chat choices grant only that sealed action, the resumed task, or the same chat session. One-action scope re-arms the gate for later actions; task scope bypasses it only during that resumed run, and chat scope is reconstructed only from a resolved card bound to the exact session id. The browser never receives selected tools/query and submits only once/task/chat/deny. New normal turns and superseding actions retire unresolved approvals without clearing taint.
