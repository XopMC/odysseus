...
def test_canonical_compact_resolves_with_owner_scope():
    body = _function_source("routes/session_routes.py", "compact_session")
    assert "owner = getattr(session, \"owner\", None) or effective_user(request)" in body
