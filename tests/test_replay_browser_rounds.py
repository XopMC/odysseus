...
try {
 ...
 for(const viewport of [{width:1280,height:900},{width:390,height:844}]) {
  ...
  await page.evaluate(({source})=>{
   const noop=()=>{},API_BASE='',TOOL_ELAPSED_TICK_MS=250,_resumingStreams=new Map(),_streamRunIds=new Map(),_streamGenerations=new Map([['s1',1]]);
   ...
   const updateSubmitButton=noop,applyStreamContextUsage=noop,displayMetrics=noop,bindUiText=noop,_touchStreamActivity=noop;
   const _flushIncrementalStreamRender=noop,_cancelIncrementalStreamRender=noop,_cancelIncrementalStreamTree=noop;
   const _queueIncrementalStreamRender=(node,text)=>{node.textContent=text;};
   const _ensureStreamLayout=body=>body.querySelector('.stream-content')||body.appendChild(document.createElement('div'));
   const _lazyToolOutputMarkup=event=>event.output?'<details class="agent-tool-output"><summary>Output</summary><pre>'+event.output+'</pre></details>':'';
   const _bindLazyToolOutput=noop;
   const timelineReducer={apply:()=>({accepted:true})};
  ...
  await page.evaluate(()=>pushReplay([{id:1,data:{delta:'Первый шаг'}},{id:2,data:{type:'tool_start',tool:'bash',command:'echo ok'}},{id:3,data:{type:'tool_output',tool:'bash',output:'ok',exit_code:0}},{id:4,data:{type:'agent_step'}},{id:5,data:{delta:'Второй шаг'}}]));
      try { await page.waitForFunction(()=>document.querySelectorAll('#chat-history > .msg').length===2,null,{timeout:2000}); }
      catch(error) { const outcome=await page.evaluate(()=>Promise.race([replay.then(value=>({value})).catch(reason=>({error:reason?.message||String(reason)})),new Promise(resolve=>setTimeout(()=>resolve({pending:true}),10))]));throw new Error(error.message+' errors='+JSON.stringify(errors)+' outcome='+JSON.stringify(outcome)+' html='+await page.locator('#chat-history').innerHTML()); }
