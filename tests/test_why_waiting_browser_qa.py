...
def test_stalled_model_panel_matches_after_two_client_reload():
    ...
    with tempfile.TemporaryDirectory(prefix="odysseus-wait-browser-qa-") as data_dir:
        ...
        env.update({
            ...
            "ODYSSEUS_REPLAY_QA_START_RUN": "0",
        ...
        try:
            ...
            stalled = {
                "run_id": "a" * 32, "run_status": "running", "goal_status": "active",
                "phase": "model", "phase_seconds": 700, "model": "fixture-model",
                "endpoint_id": "fixture-endpoint", "tool": None,
                "lease": {"held": True, "expires_at": "2026-09-23T10:00:00"},
                "checkpoint": {"durable_seq": 51, "context_revision": 7,
                               "ledger_hash": "b" * 64},
                "stalled": True, "recovery_action": "inspect",
            }
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    for viewport in ({"width": 1280, "height": 800},
                        ...
                        page.route("**/api/chat/work/*/why-waiting", lambda route: route.fulfill(
                            status=200, content_type="application/json", body=json.dumps(stalled),
                        ))
                        # This fixture has no real model/research run. Supply
                        # their idle status so unrelated expected 404s do not
                        # masquerade as waiting-panel console failures.
                        for pattern in ("**/api/chat/stream_status/*", "**/api/research/status/*"):
