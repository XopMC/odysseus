...
class RunnerTests(unittest.TestCase):
    ...
    def test_model_policy_blocks_credentials_nested_search_and_outside_write(self):
        ...
        subprocess.run(['git', '-C', str(workspace), 'add', 'ordinary.txt'], check=True)
        subprocess.run(['git', '-C', str(workspace), '-c', 'user.name=Fixture',
                        '-c', 'user.email=fixture@example.test', 'commit', '-qm', 'initial'], check=True)
        (workspace / 'ordinary.txt').write_text('changed ordinary')
        git_status = file_call('git_status', {})
        self.assertTrue(git_status['ok'], git_status)
        self.assertEqual(git_status['result']['exit_code'], 0)
        self.assertIn('ordinary.txt', [item['path'] for item in git_status['result']['files']])
        self.assertNotIn('.env', [item['path'] for item in git_status['result']['files']])
        self.assertEqual(file_call('git_diff', {'file': 'ordinary.txt'})['result']['exit_code'], 0)
        self.assertEqual(file_call('git_diff', {'file': '.env'})['result']['exit_code'], 1)
        self.assertEqual(file_call('git_log', {'limit': 1})['result']['commits'][0]['subject'], 'initial')
