    ...
    script = r"""
      ...
      if(scenario.startsWith('idle_')||scenario==='hidden_focus'){
        ...
        if(scenario==='resume_lock'){
          ...
          const toolOne=box.children[1].children[0];
          const nextBody=box.children[2].querySelector('.body');
          const toolTwo=box.children[3].children[0];
          assert.equal(box.children[1].className.includes('agent-thread'),true);
          assert.equal(box.children[3].className.includes('agent-thread'),true);
          assert.equal(box.children[1].children.length,1,'duplicate SSE IDs are ignored');
          assert.equal(toolOne.innerHTML.includes('example'),true,'completed tool output stays in its timeline node');
          assert.equal(toolTwo.querySelector('.agent-thread-tail').textContent,'Waiting for search','progress updates the live tool node');
