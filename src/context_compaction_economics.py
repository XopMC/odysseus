...
def decide(*, at_boundary: bool, used_tokens: int, input_budget: int,
           ...
           cache_debt_tokens: int = 0) -> Decision:
    ...
    rewrite = max(256, int(used * 0.30)) + max(0, int(cache_debt_tokens))
    savings = max(0, used - target) * horizon
    if used >= int(budget * 0.92):
        return Decision(True, "window_protection", target, savings, rewrite, horizon)
    if not at_boundary:
        return Decision(False, "awaiting_plan_boundary", target, savings, rewrite, horizon)
    if tokens_since_boundary < max(512, int(budget * 0.03)):
        return Decision(False, "insufficient_new_context", target, savings, rewrite, horizon)
    if savings <= rewrite:
        return Decision(False, "rewrite_not_economic", target, savings, rewrite, horizon)
    return Decision(True, "economic_plan_boundary", target, savings, rewrite, horizon)
