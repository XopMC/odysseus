...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "read_file": "Read a bounded text file. For focused reads use JSON args: path, offset (1-based line), limit, line_numbers; or byte_offset (0-based) + byte_limit (mutually exclusive with line ranges). Returns full-file SHA-256, size, encoding, binary and truncation metadata. Large UTF-8 files return a short preview plus a read_tool_artifact handle when owner-scoped storage is available; page it with read_tool_artifact instead of requesting megabytes inline. Pass sha256 to edit_file as its precondition.",
    ...
    "search_files": "Search code with ripgrep semantics and the same workspace/sensitive-file policy as grep. JSON args: pattern, optional path/glob/ignore_case, mode=files (default, concise distinct file paths only) or mode=matches (exact file:line:match results), plus cursor and page_size (1–50) for stable pagination. Continue with the returned next_cursor; do not request match bodies until needed.",
    "list_tree": "Show a bounded directory hierarchy and file sizes without opening file bodies. Honors .gitignore and skips hidden, generated, symlink and sensitive paths. Defaults to depth 2/100 entries; max depth is 6 and max entries 200. Use this to orient before reading files.",
    "file_outline": "Show Python/Python-stub AST classes, functions and methods with exact start/end line numbers without returning source text. Supports regular UTF-8 Python files up to 2 MiB, 100 symbols by default/200 max. Other languages return explicit unavailable; do not infer symbols from unsupported files.",
    ...
    "run_tests": "Set profile=list to discover available pytest/npm_test profiles without executing code; otherwise run one discovered profile in the allowed workspace with bounded output, timeout, exact exit code and failure artifact. Never accepts arbitrary commands. Prefer over bash for standard project tests.",
    "run_lint": "Set profile=list for read-only npm_lint discovery, or run the discovered lint profile with bounded output, timeout and exact exit code. Never accepts arbitrary commands. Prefer over bash for standard project lint.",
    ...
    "rollback_file_checkpoint": "Roll back an Agent/Goal file mutation only from its returned checkpoint_id and exact expected_sha256 map. Works on local container and registered Jetson host file changes; preserves intervening user edits when any path hash or identity changed.",
    ...
    "create_plan": "Create a durable, owner-scoped read-only plan with stable steps. This proposes work only; execution starts after explicit user approval.",
    "update_plan_step": "Update one stable step of the approved durable plan with pending, in-progress, done or blocked state and verification summary.",
    "get_goal": "Read the active durable Goal objective, progress, attempt and checkpoint for this chat.",
    "update_goal_progress": "Checkpoint concrete progress on the active Goal without completing it. Record verified facts, pending actions and blockers.",
    "complete_goal": "Complete an active Goal only after all requirements are satisfied, with a concise summary and concrete verification evidence.",
